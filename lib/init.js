...
const getConnectionValues = async buildId => {
  if (process.env.DOLOS_SQL_CONNECTION) {
    ...
    return JSON.parse(process.env.DOLOS_SQL_CONNECTION)
  }
...
const init = async () => {
  const buildId = process.env.DOLOS_SQL_IS_PROD ? 'prod' : nanoid()
  ...
  await setEnv(envFilePath, {
    ...env,
    DOLOS_SQL_CONNECTION: JSON.stringify(connectionValues)
  })
